...
import { PlayersSeasonStatsComponent } from './players-season-stats.component';
...
describe('PlayersSeasonStatsComponent', () => {
  ...
  const renderComponent = () =>
    render(PlayersSeasonStatsComponent, {
      inputs: { seasonId },
      providers: [
  ...
  describe('with non-dummy team', () => {
    it('renders team image', async () => {
      const stat = { ...fakePlayerSeasonStat(), team: { ...fakeTeamBase(), dummy: false } };
      mockApi.getPlayerSeasonStatsBySeasonId.mockReturnValue(of(onePage([stat])));
      await renderComponent();

      await waitFor(() => {
        expect(screen.getByAltText(stat.team.name)).toBeInTheDocument();
      });
    });
  });

  describe('with dummy team', () => {
    it('does not render team image', async () => {
      const stat = { ...fakePlayerSeasonStat(), team: { ...fakeTeamBase(), dummy: true } };
      mockApi.getPlayerSeasonStatsBySeasonId.mockReturnValue(of(onePage([stat])));
      await renderComponent();

      await waitFor(() => {
        expect(screen.queryByAltText(stat.team.name)).not.toBeInTheDocument();
      });
    });
  });

  describe('with non-dummy d11Team', () => {
    it('renders d11Team name', async () => {
      const stat = { ...fakePlayerSeasonStat(), d11Team: { ...fakeD11TeamBase(), dummy: false } };
      mockApi.getPlayerSeasonStatsBySeasonId.mockReturnValue(of(onePage([stat])));
      await renderComponent();

      await waitFor(() => {
        expect(screen.getByText(stat.d11Team.name, { exact: false })).toBeInTheDocument();
      });
    });
  });

  describe('with dummy d11Team', () => {
    it('does not render d11Team name', async () => {
      const stat = { ...fakePlayerSeasonStat(), d11Team: { ...fakeD11TeamBase(), dummy: true } };
      mockApi.getPlayerSeasonStatsBySeasonId.mockReturnValue(of(onePage([stat])));
      await renderComponent();

      await waitFor(() => {
        expect(screen.queryByText(stat.d11Team.name, { exact: false })).not.toBeInTheDocument();
      });
    });
  });
  ...
  describe('filters', () => {
    ...
    it('calls api with RATING sort', async () => {
    ...
    it('calls api with FORM sort', async () => {
