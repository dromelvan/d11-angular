...
import { SeasonWinnersSectionComponent } from './season-winners-section.component';
...
describe('SeasonWinnersSectionComponent', () => {
  beforeEach(async () => {
    vi.clearAllMocks();

    await render(SeasonWinnersSectionComponent, {
      providers: [{ provide: RouterService, useValue: mockRouterService }],
      inputs: { seasonWinners: seasonWinner },
    });
  });

  it('renders season name as section header', () => {
    expect(screen.getByTestId('section-header')).toHaveTextContent(seasonWinner.season.name);
  });
...
describe('SeasonWinnersSectionComponent with positive goal difference', () => {
  beforeEach(async () => {
    await render(SeasonWinnersSectionComponent, {
      providers: [{ provide: RouterService, useValue: mockRouterService }],
      inputs: {
...
describe('SeasonWinnersSectionComponent with active season', () => {
  beforeEach(async () => {
    await render(SeasonWinnersSectionComponent, {
      providers: [{ provide: RouterService, useValue: mockRouterService }],
      inputs: {
...
describe('SeasonWinnersSectionComponent with 2nd win', () => {
  beforeEach(async () => {
    await render(SeasonWinnersSectionComponent, {
      providers: [{ provide: RouterService, useValue: mockRouterService }],
      inputs: {
...
describe('SeasonWinnersSectionComponent with pending season', () => {
  beforeEach(async () => {
    await render(SeasonWinnersSectionComponent, {
      providers: [{ provide: RouterService, useValue: mockRouterService }],
      inputs: {
